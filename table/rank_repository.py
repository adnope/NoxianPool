...
class RankRepository:
    ...
    def get_rank(self, rank_id: int) -> Optional[Rank]:
        data = self.client.table("ranks").select("*").eq("id", rank_id).single().execute()
        return Rank(id=data["id"], rank=data["rank"]) if data else None

    def delete_rank(self, rank_id: int):
        self.client.table("ranks").delete().eq("id", rank_id).execute()

    def list_ranks(self) -> List[Rank]:
        data = self.client.table("ranks").select("*").execute()
        return [Rank(id=item["id"], rank=item["rank"]) for item in data]